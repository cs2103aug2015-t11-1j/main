...
public class Task {
	...
	public String toStorage() {
		if(_time.equals(MESSAGE_SYMBOL_NOTHING)){
			if(_date.equals(MESSAGE_SYMBOL_NOTHING)){
				return _index + MESSAGE_SYMBOL_SPACE + _stats + MESSAGE_SYMBOL_HYPHEN_SPACE + MESSAGE_SYMBOL_HYPHEN_SPACE + _details;
			}
	...
	public String rotateDF() {
		if (_date.length() > INT_ONE) {
			String str = new String(this._date.substring(INT_SIX, INT_EIGHT) + this._date.substring(INT_TWO, INT_SIX) + this._date.substring(INT_ZERO, INT_TWO) + MESSAGE_SYMBOL_SPACE);
			return str;
		}
		return _date;
	}
